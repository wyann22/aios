# Lesson 3：从单文件到推理框架 — 将 run_qwen3.py 重构为 aios/ 包
...
```
改造前：run_qwen3.py (251行，单文件)                改造后：python/aios/ (20个文件，模块化包)
─────────────────────────────                ─────────────────────────────────
...
**对应 run_qwen3.py 中的**：所有 `nn.Module` 继承
...
**对应 run_qwen3.py 中的**：所有 `nn.Linear(..., bias=False)`

**改造对比**：

```python
# ❌ run_qwen3.py（nn.Module 版本）
...
**对应 run_qwen3.py 中的**：`class RMSNorm(nn.Module)`
...
**对应 run_qwen3.py 中的**：`class RotaryEmbedding(nn.Module)` + `register_buffer`

这是改动最大的一个层，因为 run_qwen3.py 用了 `register_buffer` 存放 `inv_freq`：
...
**关键变化**：
- run_qwen3.py：每次 forward 都现算 cos/sin → 预计算所有位置，存为 cache
...
**对应 run_qwen3.py 中的**：`apply_rotary_pos_emb()`、`rotate_half()`、`repeat_kv()`

这些纯函数直接从 run_qwen3.py 搬出来，**几乎无改动**：
...
**对应 run_qwen3.py 中的**：`nn.Embedding` 和 `nn.Linear`（用作 lm_head）
...
与 run_qwen3.py 的 `Qwen3ForCausalLM(nn.Module)` 对比，这里用 `BaseLLMModel(ABC, BaseOP)` 作为所有 LLM 模型的抽象基类。后续支持其他模型（如 LLaMA）时，只需新增一个继承 `BaseLLMModel` 的类。

---

### 第 9 步：消除 transformers.AutoConfig — `models/config.py`

**对应 run_qwen3.py 中的**：`config = AutoConfig.from_pretrained(args.model)`

run_qwen3.py 用 `AutoConfig` 读取模型配置，这依赖 `transformers` 库。实际上配置就是模型目录下的一个 `config.json` 文件：
...
**对应 run_qwen3.py 中的**：`Qwen3Attention`、`Qwen3MLP`、`Qwen3DecoderLayer`、`Qwen3Model`、`Qwen3ForCausalLM`

这是代码量最大的一步（~120 行），但逻辑与 run_qwen3.py **完全一致**，只是基类和调用方式变了。
...
**对应 run_qwen3.py 中的**：`load_weights_from_hf()` 函数

这是消除 `transformers` 依赖（除 tokenizer 外）的关键步骤。

```python
# ❌ run_qwen3.py — 需要加载两份模型！
...
**对应 run_qwen3.py 中的**：`generate()` 函数的 `temperature`、`top_k` 等散落参数
...
**对应 run_qwen3.py 中的**：`main()` 函数 + `generate()` 函数

这是整个架构的入口。将 run_qwen3.py 的 `main()` 中的初始化逻辑和 `generate()` 函数合并为一个 `LLM` 类：
...
class LLM:
    ...
    def generate(self, prompts, sampling_params=None):
        # 自回归生成逻辑与 run_qwen3.py 的 generate() 完全一致
        ...
```
...
**总计 ~700 行**（vs run_qwen3.py 的 251 行），增加的是模块化结构和注释，推理逻辑零改动。
...
#   ✓ 输出与 run_qwen3.py 一致（相同 prompt + greedy → 相同输出）
